...
> **재현 가능성**: 이 문서대로 실행하면 동일한 `courses_kr_production.json` 재생성 가능  
> **마지막 빌드**: 2026-05-11
...
│  Stage 5: JSON 출력                                 │
│  ───────────────────────────────────────────────   │
│  • courses_kr.json (전체 + 디버그)                 │
│  • courses_kr_production.json (앱 번들용)          │
...
**OSM에서 한국 골프장의 홀별 매핑은 매우 미진합니다.** 18홀 전체가 매핑된 골프장은 단 3곳입니다.

| 상태 | 개수 | 비율 |
|------|------|------|
| 18홀 완전 매핑 | 3 | 0.5% |
| 9홀 이상 매핑 | 14 | 2.6% |
| 골프장 위치만 (홀 정보 없음) | 524 | 95.9% |
...
| 앱 기능 | 영향 |
|---------|------|
| F1 골프장 자동 매칭 | ✅ **거의 모든 골프장(546개)에서 정상 작동** |
| F3 GPS 홀 자동 감지 | ⚠️ 14개 골프장에서만 작동 — 나머지는 수동 모드 fallback |
| F4 타수 카운터 | ✅ 데이터와 무관, 항상 동작 |

> MVP 출시에는 충분합니다. F3는 "베타 기능"으로 안내하고, 사용자 제보로 점진 개선 가능.

---

## 9. 향후 데이터 보강 방안

### 9.1 한국 공공데이터 통합 (우선순위 1)

**현재 본 환경에서는 data.go.kr / localdata.go.kr의 SSL이 차단**되어 자동 다운로드 불가합니다.  
사용자가 브라우저로 직접 다운로드 후 빌드 환경에 업로드하면 통합 가능.
...
### 9.2 카카오/네이버 맵 API (우선순위 2)

API 키 필요. 골프장 이름 검색 → 정확한 좌표·전화·주소 보강.
