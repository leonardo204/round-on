...
│  Stage 7: JSON 출력                                 │
│  ───────────────────────────────────────────────   │
│  • courses_seed_v3.json (앱 번들용, 1,163곳)       │
...
> **참고**: 아래 출력 샘플은 v2 빌드(OSM 전용, 623개) 기준의 역사적 참조입니다. v3(2026-05-12)는 1,163개, courseType=course 기준 1,163개 전체를 포함합니다.
...
[4/5] 골프장 레코드 빌드...
  최종 골프장 레코드: 623개  ← v2 OSM 전용 수치 (v3: 1,163개)

[5/5] JSON 저장...
  ✅ courses_kr.json 저장 완료

=== 데이터 품질 리포트 (v2 기준, 역사적 참조) ===
전체 골프장: 623개  ← v2 수치 (v3: 1,163개)

시설 타입 (v2):
  course: 546개  ← v2 수치 (v3에서는 공공데이터+카카오 enrichment로 1,163개)
  practice: 57개
  park_golf: 14개
  screen: 6개
...
**OSM에서 한국 골프장의 홀별 매핑은 매우 미진합니다.** 18홀 전체가 매핑된 골프장은 단 3곳입니다. v3는 공공데이터 + 카카오 enrichment로 전체 커버리지를 1,163곳으로 확대했습니다.

| 상태 | 개수 | 비율 |
|------|------|------|
| 18홀 완전 매핑 (complete) | 3 | 0.26% |
| 9홀 이상 매핑 (partial) | 12 | 1.0% |
| 1~8홀 매핑 (minimal) | 9 | 0.8% |
| 골프장 위치만 (low) | 1139 | 97.9% |
...
| 앱 기능 | 영향 |
|---------|------|
| F1 골프장 GPS 자동 매칭 | ✅ **모든 1,163곳에서 정상 작동** |
| F3 골프장+서브코스 GPS 감지 | ✅ 모든 1,163곳에서 골프장 단위 감지 동작 (홀 단위 자동 감지 미제공) |
...
**v3에서 문체부 + 행안부 + 카카오 로컬 API 다단계 통합 완료.** 546곳(OSM) → 1,163곳(v3)으로 확대.  
카카오 enrichment: 820/1,163 골프장에 phone/kakaoPlaceUrl 채워짐.
