...
- 앱 실행 시 GPS로 한국 골프장 자동 매칭 (반경 3km, 546개 DB) (spec_3.md:58-61)
...
| 한국 골프장 자동 매칭 | 앱 실행 시 GPS로 546개 한국 골프장 DB와 즉시 매칭, 수동 변경 가능 | spec_3.md:57-62 |
...
546개 골프장 데이터 중 524개는 `dataQuality: low` (클럽하우스 좌표만 보유). GPS 홀 자동 감지(F3)는 홀별 티박스·그린 좌표가 있는 **14개 코스에서만 동작**. 나머지는 수동 홀 선택 fallback 처리 필수. (CLAUDE.md §PROJECT)
