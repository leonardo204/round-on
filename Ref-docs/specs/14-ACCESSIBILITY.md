...
| DataQualityBadge | "GPS 홀 자동 감지 {지원/부분지원/미지원/미정} 코스" | — | — | `.staticText` | 11-COMPONENTS §9 |
...
F3 GPS 홀 자동 전환 시 (13-HAPTICS_AND_MOTION §3 `.start` 햅틱과 동시):
...
|------|------|----------------|----------|
| high | `primary #7FB069` | `checkmark.circle.fill` | "GPS 홀 자동 감지 지원 코스" |
| medium | `text-secondary` | `exclamationmark.circle` | "GPS 홀 자동 감지 부분 지원 코스" |
| low | `text-secondary` | `xmark.circle` | "GPS 홀 자동 감지 미지원 코스" |
| unknown | `text-secondary` | `questionmark.circle` | "GPS 홀 자동 감지 분류 미정 코스" |
