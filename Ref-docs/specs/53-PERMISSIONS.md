...
> **상태**: DRAFT · SPEC-UNDEFINED 항목 5개 미확정 · 2026-05-13 작성

> **관련 문서**: [01-SPEC](01-SPEC.md) · [02-USER_FLOWS](02-USER_FLOWS.md) · [21-DATA_MODEL](21-DATA_MODEL.md) · [33-SECURITY](33-SECURITY.md) · [50-PRIVACY_POLICY](50-PRIVACY_POLICY.md) · [전체 인덱스](README.md)

> **작성일**: 2026-05-11
> **버전**: v4 기반
> **출처 명세서**: [기능 명세서 v4](01-SPEC.md) §9 (01-SPEC.md:655-672)
> **관련 문서**: `02-USER_FLOWS.md` (권한 거부 분기), `21-DATA_MODEL.md` (Player.name 제약), `30-API_SPEC.md` (viewer TTL), `31-VIEWER_HTML.md` (책임 경계), `33-SECURITY.md`

> **[spec 미정의 항목 일람]**
>
> 아래 5개는 `01-SPEC.md §9`에 명시 없음. 본 문서가 보강 제안하며, 구현 권장안은 각 항목이 등장하는 절에 `[SPEC-UNDEFINED]` 라벨로 표기한다.
>
...
01-SPEC.md §9 (01-SPEC.md:657-664) 기준 6개 권한과 본 시리즈 범위 외 1개를 포함한다. 각 행의 "spec 출처" 컬럼은 01-SPEC.md 라인 번호로 1:1 추적한다.

| 권한 | Info.plist 키 | 요청 시점 | 거부 시 동작 | spec 출처 |
|------|--------------|---------|-----------|---------|
| 위치 (When In Use) | `NSLocationWhenInUseUsageDescription` | 앱 첫 실행 (F-A) | 수동 검색 fallback (02-USER_FLOWS L49) | 01-SPEC.md:657-659 |
| HealthKit Workout | `NSHealthShareUsageDescription` + `NSHealthUpdateUsageDescription` [SPEC-UNDEFINED 분리] | 라운드 시작 (F-B) | 워크아웃 없이 점수만 기록 (02-USER_FLOWS L69) | 01-SPEC.md:660 |
| 사진 라이브러리 (선택 접근) | `NSPhotoLibraryUsageDescription` | 사진 첨부 시 (F9) | 카메라만 노출 | 01-SPEC.md:661 |
| 카메라 | `NSCameraUsageDescription` | "촬영" 옵션 탭 시 (F9) | 라이브러리만 노출 | 01-SPEC.md:662 |
| 알림 | (Info.plist 키 불필요, 런타임 호출) | 라운드 시작 후 | 무음 동작, 기능 영향 없음 [SPEC-UNDEFINED fallback] | 01-SPEC.md:663 |
| iCloud / CloudKit | (시스템 계정 자동, Info.plist 키 불필요) | 첫 SwiftData write | 로컬 only [SPEC-UNDEFINED UI] | 01-SPEC.md:664 |
| **본 시리즈 범위 외** | `NSMotionUsageDescription` | — | — | 01-SPEC.md 미정의 → `13-HAPTICS_AND_MOTION.md` 도입 시 보강 |

**매트릭스 독해 가이드**

- "Info.plist 키 불필요" 행은 런타임 API 호출만으로 처리되며 Info.plist 등록 시 심사 거부 가능
- [SPEC-UNDEFINED] 라벨이 붙은 셀은 본 문서 해당 절에서 구현 권장안을 제시
- "본 시리즈 범위 외" 행은 01-SPEC.md에 정의 없는 권한. 13-HAPTICS_AND_MOTION.md 도입 전까지 Info.plist에 추가하지 않는다
...
- 관련 기능: F1 골프장 자동 매칭, F3 GPS 자동 감지 — 골프장 + 서브코스 단위 (홀 단위 자동 감지는 미제공) (01-SPEC.md:57-62, 70-77)
- 요청 API: `CLLocationManager.requestWhenInUseAuthorization()` (02-USER_FLOWS L41)
- 요청 시점: 앱 첫 실행 F-A 플로우 진입 시 (02-USER_FLOWS L41-44)
- 위치 사용 빈도: 앱 실행 시 1회 fetch (F1), 라운드 중 2분 폴링 또는 사용자 인터랙션 시 (F3, 01-SPEC.md:73)
- Always On 권한 (`NSLocationAlwaysAndWhenInUseUsageDescription`)은 요청하지 않음 — 배터리 절약 원칙 (01-SPEC.md:73)

**[SPEC-UNDEFINED] iOS 14+ Precise / Approximate Location 정책 분기**

- 골프장 반경 3km 매칭 및 홀 감지는 Precise Location 필요 (01-SPEC.md:59)
- 사용자가 Approximate 선택 시: 매칭 정확도 저하 가능, 수동 검색 권장 안내 필요
- 구현 권장: `CLLocationManager.accuracyAuthorization` 확인 후 UI 상태 반영
- dataQuality: low 코스(965곳 중 941곳)에서는 홀 감지 비활성 — 위치 정확도 분기와 독립적으로 처리 (CLAUDE.md §PROJECT, 02-USER_FLOWS L51-52)
...
**거부 fallback**: 수동 골프장 검색/선택 화면으로 전환 (01-SPEC.md:634, 02-USER_FLOWS L49)

---

## 3. HealthKit 권한 (NSHealthShareUsageDescription + NSHealthUpdateUsageDescription)

- 관련 기능: F8 Apple Health 연동 (01-SPEC.md:116-119, 660)
- 요청 시점: 라운드 시작 "라운드 시작" 탭 후 HealthKit 미허가 시 (02-USER_FLOWS L63)

**[SPEC-UNDEFINED] Share vs Update 키 분리 보강**

01-SPEC.md §9 (01-SPEC.md:660)는 "HealthKit Workout Write"만 명시한다. 본 문서는 보수적으로 두 키 모두 권장한다.
...
두 키를 동시에 선언해야 `HKHealthStore.requestAuthorization(toShare:read:)` 호출이 유효하다. Write 전용으로 선언해도 심사 통과는 가능하나, F8 심박/칼로리 표시(01-SPEC.md:118)를 위해 Read 포함을 권장한다.
...
**거부 fallback**: 워크아웃 세션 없이 점수 기록만 진행 (01-SPEC.md:116-119, 02-USER_FLOWS L69)

---

## 4. 사진 라이브러리 권한 (NSPhotoLibraryUsageDescription)

- 관련 기능: F9 사진 첨부 — 카메라 롤에서 선택 (01-SPEC.md:134-138, 661)
- 요청 시점: 사진 첨부 메뉴 → "라이브러리" 옵션 탭 시
- iOS 14+ Limited Photos Access (`PHPickerViewController`) 권장 — 01-SPEC.md:661 "선택 접근" 준수
...
viewer 다운로드 기능(01-SPEC.md:137 "방문자가 사진 다운로드 가능")을 iOS 앱 내에서 구현하거나, viewer 링크 저장 화면을 추가할 경우 `NSPhotoLibraryAddUsageDescription` 키가 별도로 필요하다. 보수적으로 함께 선언을 권장하며, 사용하지 않을 경우 App Store Connect에서 키 삭제 가능하다.
...
**거부 fallback**: 카메라 즉석 촬영 옵션만 노출 (01-SPEC.md:135)

---

## 5. 카메라 권한 (NSCameraUsageDescription)

- 관련 기능: F9 라운드 중 즉석 촬영 (01-SPEC.md:135, 662)
- 요청 시점: 사진 첨부 메뉴 → "촬영" 옵션 탭 시 (01-SPEC.md:662)
...
**거부 fallback**: 라이브러리 선택 옵션만 노출 (01-SPEC.md:135)

---

## 6. 알림 권한 (UNUserNotificationCenter)

- 관련 기능: F3 GPS 자동 감지 — 골프장+서브코스 감지 완료 알림 (01-SPEC.md:639-643, 663)
- Info.plist 키 불필요 — 런타임 `UNUserNotificationCenter.requestAuthorization(options:)` 호출 (01-SPEC.md:663)
- 요청 옵션: `.alert`, `.sound` 최소 범위 권장 (배지는 라운드 특성상 불필요)
- 요청 시점: 라운드 시작 후 적절한 시점 (첫 홀 진입 전 또는 F-B 완료 직후 권장)
- 용도: 골프장+서브코스 GPS 감지 완료 알림, Watch 미착용 시 iPhone 알림 (홀 단위 자동 감지는 미제공)
- F3 GPS 자동 감지는 965곳 전체에서 골프장 단위로 동작 (CLAUDE.md §PROJECT, 02-USER_FLOWS L52)

**[SPEC-UNDEFINED] 알림 권한 거부 시 F3 fallback UI**

01-SPEC.md §9 (01-SPEC.md:663)는 거부 시 동작을 명시하지 않는다. 구현 권장:

- 알림 권한 거부 시 시스템 알림 대신 앱 내 토스트/배너로 홀 전환 안내
- F3 GPS 홀 감지 자체(01-SPEC.md:70-77)는 알림 권한과 무관하게 동작
...
- 관련 기능: F7 CloudKit sync (01-SPEC.md:112-114, 664)
- Info.plist 키 불필요 — 시스템 iCloud 계정 기반 자동 처리 (01-SPEC.md:664)
- SwiftData + CloudKit 연동: 첫 SwiftData write 시점에 내부적으로 권한 협상

**iCloud 미로그인 / iCloud Drive 비활성 시**

- 동작: 로컬 SwiftData만 활성화, 멀티 디바이스 sync 비활성 (01-SPEC.md:114)
...
- **위치**: 기기 외부 전송 절대 금지. 골프장 매칭 및 홀 감지에만 사용 (01-SPEC.md:667, CLAUDE.md §PROJECT)
- **동반자 이름**: 별명만 허용 (Player.name 모델 제약 — 21-DATA_MODEL.md). 실명 및 연락처 업로드 금지 (01-SPEC.md:282, 670, CLAUDE.md §PROJECT)
- **viewer 데이터 및 사진**: 공유 시점으로부터 7일 후 Cloudflare KV/R2 자동 삭제 (01-SPEC.md:669, CLAUDE.md §PROJECT, 30-API_SPEC §8)
- **디바이스 토큰**: 익명 UUID — 외부 식별 불가, 서버 측 PII 연결 없음 (01-SPEC.md:670)
