...
| F3 골프장 GPS 매칭 (서브코스 감지 포함) | `.start` + 시각 토스트 | `.success` (Notification) | spec_3.md §7.3 | 골프장+서브코스 단위 감지 완료 알림; 홀 단위 자동 전환은 미제공 |
...
| 양쪽 동시 발화 (F3 골프장 GPS 매칭, F5 라운드 종료) | `.start` / `.stop` | `.success` (Notification) | 사용자가 어느 디바이스를 보고 있어도 이벤트를 인지 가능 (spec_3.md:908) |
