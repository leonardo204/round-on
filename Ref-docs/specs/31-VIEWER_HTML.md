...
>
> ⚠️ **2026-05-18 폐기 사항**: 사진 갤러리(§5), 사진 저장 흐름(§6), 라이트박스(§7) 모두 제거됨. 현재 viewer HTML은 **헤더 + 스코어카드(OUT/IN) + 푸터(만료/OSM ODbL)** 만 렌더링. 본 문서의 §5~§7 및 사진 관련 모든 명세는 무효이며 별도 patch 없이 폐기됨으로 간주.
