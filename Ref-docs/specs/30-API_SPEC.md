...
> 아래 5개 엔드포인트는 "통계 공유 v1" 기능에 해당한다. 라운드 공유(`/api/share`)와 같은 3토큰 모델을 사용하되 KV namespace와 shortId prefix를 분리한다.
...
**페이로드 크기 한도**: 1MB (`Content-Length` 초과 시 413)
...
> **참고**: 라운드 공유와 달리 통계 공유에는 사진/R2 자원이 없으므로 KV 키 1개만 삭제한다.
