...
> **작성일**: 2026-05-11
> **버전**: v4 기반
> **출처 명세서**: [기능 명세서 v4](../golf-scorecard-app-spec_3.md) §3.3 (spec_3.md:259-284), §9 (spec_3.md:655-672)
> **관련 문서**: `21-DATA_MODEL.md` (SwiftData 모델), `31-VIEWER_HTML.md` (작성 예정), `32-CLOUDFLARE_SETUP.md` (작성 예정), `33-SECURITY.md` (작성 예정)

> **[spec 미정의 5건 일람]**
>
> 아래 5개 항목은 `spec_3.md`에 설계 의도만 있고 구체적 명세가 없다. 본 문서에서 각 항목이 등장하는 곳에 `[SPEC-UNDEFINED]` 라벨을 표기하며, 구현 권장안은 §9에 모아 두었다.
>
...
| 항목 | 값 | 출처 |
|------|----|------|
| Base URL | `https://golf.zerolive.co.kr` | spec_3.md:6 |
| 전송 프로토콜 | HTTPS only (Cloudflare 자동 적용) | spec_3.md:281 |
| 기본 Content-Type | `application/json; charset=UTF-8` | (사진 업로드 제외) |
| 사진 업로드 Content-Type | `multipart/form-data` [SPEC-UNDEFINED: 권장] | §9.1 참조 |

### 2.2 3토큰 분리 모델

spec_3.md §3.4의 보안 설계는 세 가지 독립 토큰으로 구성된다. (spec_3.md:277-278, 670)

| 토큰 | 역할 | 발급 주체 | 전달 방식 | 출처 |
|------|------|----------|---------|------|
| `deviceToken` | 발급자 추적 (익명 UUID). Rate limiting 기준 | 앱 최초 실행 시 생성 — 외부 노출 없는 익명 UUID | 요청 바디 | spec_3.md:670 |
| `editToken` | 수정 권한 인증 (PUT/DELETE 헤더 필수). KV에 저장, 디바이스 외부 노출 금지 | Worker가 POST /api/share 응답 시 생성 | `Authorization` 요청 헤더 | spec_3.md:277 |
| `PIN` | viewer 접근 제어 (4자리 숫자). bcrypt 해시로 KV 저장 | 공유 생성 시 사용자가 선택 입력 | 요청 바디 (`pin` 필드) | spec_3.md:278 |
...
라운드 데이터를 Cloudflare KV/R2에 저장하고 shortId + URL + editToken을 반환한다. (spec_3.md:262-264)
...
- `deviceToken`: 익명 UUID, Rate limiting 기준 (spec_3.md:670)
- `round`: SwiftData Round 객체의 JSON 직렬화 **[SPEC-UNDEFINED: 페이로드 구조 미정의 — §9.3 참조]**
- `options.nameVisibility`: `"real"` 실명 / `"anonymous"` 익명(A/B/C/D) (spec_3.md:131)
- `options.accessControl`: `"public"` 공개 / `"pin"` PIN 보호 (spec_3.md:132)
- `options.pin`: `accessControl == "pin"` 일 때만 필수 (4자리 숫자). Worker에서 bcrypt 해시 저장 (spec_3.md:278)
...
|----------|---------------|------|------|
| `shortId` | `sharedShortId` | base62 8자 단축 ID | spec_3.md:276, 21-DATA_MODEL §8 |
| `url` | `sharedURL` | 전체 Viewer URL | spec_3.md:125, 21-DATA_MODEL §8 |
| `editToken` | `sharedEditToken` | 수정/삭제용 토큰 (로컬 저장) | spec_3.md:277, 21-DATA_MODEL §8 |
| `expiresAt` | `sharedExpiresAt` | 생성 시점 + 7일 (ISO 8601 UTC) | spec_3.md:126, 21-DATA_MODEL §8 |
...
기존 shortId를 유지하면서 메타데이터를 갱신한다. 사진 수정은 §5 엔드포인트를 사용한다. (spec_3.md:139-142, 266)
...
`round` 및 `options` 는 변경할 항목만 포함 가능 (partial update 권장). (spec_3.md:140-141)
...
사진을 R2에 저장하고 photoId를 반환한다. (spec_3.md:267)
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `photo` | 파일 (JPEG/PNG) | 업로드할 사진. 최대 10MB (spec_3.md:279) |
...
R2에서 해당 사진 파일을 삭제하고 KV 메타데이터에서 참조를 제거한다. (spec_3.md:268)
...
스코어카드 + 사진 갤러리를 포함하는 모바일 최적화 HTML을 반환한다. (spec_3.md:269)

- 응답 `Content-Type`: `text/html; charset=UTF-8`
- PIN 보호 viewer의 경우 잠금 화면을 먼저 렌더링한다. (spec_3.md:239-240)
...
img 태그에서 사용하는 인라인 사진 응답이다. (spec_3.md:270)
...
viewer에 속한 전체 사진을 ZIP으로 압축하여 스트리밍 다운로드한다. (spec_3.md:271)
...
| 401 | editToken 불일치 또는 PIN 미인증 | PUT, DELETE, PIN 보호 GET | spec_3.md:277-278 |
| 403 | 권한 부족 (드문 케이스) | — | — |
| 404 | shortId 또는 photoId 없음 | 모든 엔드포인트 | — |
| 410 | viewer 만료 (생성 후 7일 경과, KV/R2 자동 삭제) | 모든 엔드포인트 | spec_3.md:126 |
| 413 | 페이로드 초과 (사진 1장 10MB 초과 등) | POST /photos, POST /api/share | spec_3.md:279 |
| 429 | Rate limit 초과 (deviceToken 기준 1분 5건) | POST /api/share | spec_3.md:280 |
...
| 규칙 | 값 | 출처 |
|------|----|------|
| POST /api/share Rate limit | deviceToken 당 1분 5건 | spec_3.md:280 |
| POST /photos Rate limit | [SPEC-UNDEFINED: 권장안 §9.5 참조] | — |
| round JSON 최대 크기 | [SPEC-UNDEFINED: 권장 1MB] — POST /api/share 의 413 트리거 조건 | — |
| PIN 오답 잠금 | 5회 오답 시 1시간 잠금 | spec_3.md:278 |
| 사진 1장 최대 크기 | 10MB | spec_3.md:279 |
| viewer당 사진 최대 수 | 30장 | spec_3.md:279 |
| shortId 형식 | base62 8자 (218조 경우의 수) | spec_3.md:276 |
| editToken | Worker 생성 후 KV 저장. 디바이스 외부 노출 금지 | spec_3.md:277 |
| PIN 저장 방식 | bcrypt 해시 저장 (cost factor는 33-SECURITY.md 위임) | spec_3.md:278 |
| PII 차단 | 동반자 실명/연락처 업로드 거부 | spec_3.md:282, CLAUDE.md §PROJECT |
| 전송 보안 | HTTPS only (Cloudflare 자동 적용) | spec_3.md:281 |
| deviceToken | 익명 UUID — 외부 전송하지 않으며 서버 측 Rate limiting에만 사용 | spec_3.md:670 |
...
- 실패 시: 401 반환. 5회 누적 오답 시 1시간 잠금 (spec_3.md:278). 잠금 키 차원(`CF-Connecting-IP` vs 세션 토큰 vs deviceToken)은 `33-SECURITY.md` (작성 예정)에 위임.
- PIN 해시 비교는 KV에 저장된 bcrypt 해시와 대조 (spec_3.md:278)

### 9.3 round JSON 페이로드 구조 [SPEC-UNDEFINED]

POST /api/share 및 PUT /api/share/{shortId} 에서 전달하는 `round` 객체의 와이어 스키마가 spec_3.md에 정의되어 있지 않다. 최소 필드 기준: `id / date / courseName / startedAt / finishedAt / totalScore / players[] / holes[]` — `21-DATA_MODEL.md` §2의 @Model 구조를 기준으로 직렬화 코드 작성 시 확정한다.
...
spec_3.md는 `POST /api/share` Rate limit만 명시한다 (spec_3.md:280). 사진 업로드에 대한 별도 Rate limit 권장안:
...
spec_3.md에 정의 없음. 권장: `DELETE /api/share/{shortId}` + `Authorization: Bearer {editToken}` 헤더. 성공 시 KV 메타데이터 + R2 사진 전체 삭제 → 204 반환. 앱 측은 204 수신 후 Round의 `sharedShortId / sharedURL / sharedExpiresAt / sharedEditToken` 4개 필드를 nil 처리한다. (21-DATA_MODEL §8)
