...
spec §11 Step 1 (spec_3.md:847-852)에 따라 **3타깃**으로 구성한다. 빌드 시스템은 **XcodeGen** (`project.yml` → `RoundOn.xcodeproj`)을 사용하며, `courses.json` 번들 리소스는 `project.yml`의 `resources:` 섹션에 등록한다.
