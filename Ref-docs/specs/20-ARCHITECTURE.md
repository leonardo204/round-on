...
> **작성일**: 2026-05-11
> **버전**: v4 기반
> **출처 명세서**: [기능 명세서 v4](../golf-scorecard-app-spec_3.md) §3 (222-284), §6, §F7 (112-114), §F8 (116-119), §11 (843-902)
> **관련 문서**: `21-DATA_MODEL.md`, `22-STATE_MANAGEMENT.md` (작성 예정), `23-OFFLINE_BEHAVIOR.md` (작성 예정), `30-API_SPEC.md`, `31-VIEWER_HTML.md`, `32-CLOUDFLARE_SETUP.md` (작성 예정), `53-PERMISSIONS.md`

---

## 1. 개요

라운드온(Round-On)은 **iPhone 앱 + Apple Watch 앱 + Cloudflare Worker 백엔드** 세 축으로 구성되며, 본 문서는 이 세 축 간의 데이터 흐름, 모듈 분리, 계층 구조, 핵심 아키텍처 결정을 정의한다. (CLAUDE.md §PROJECT), (spec_3.md §11)
...
║  │ 메타  │       │ 사진  │                                            ║
║  └──────┘       └───────┘                                            ║
║  (모두 7일 TTL — spec_3.md:248-250)                                   ║
└─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┘
```

출처: (spec_3.md:222-284), (spec_3.md §6), (spec_3.md §F7-F8)
...
spec §11 Step 9 (spec_3.md:871-872) 매핑: "SwiftData ↔ CloudKit 자동 sync, 미완료 라운드 복구"

1. 사용자가 F-B (라운드 시작) 탭 → `Round` / `Player` / `HoleScore` SwiftData write
2. iOS 17 SwiftData가 CloudKit private DB로 백그라운드 자동 sync
   - 명세 근거: spec_3.md:112-114 "CloudKit으로 라운드 기록 자동 동기화. 로컬 우선, 네트워크 복구 시 자동 sync"
3. 네트워크 단절 시 로컬 SwiftData만 활성, 연결 복구 시 자동 sync 재개 (spec_3.md:114)
4. 앱 강제 종료 후 재실행 시 진행 중 라운드 자동 복구 (spec_3.md:108-109)

충돌 해결 알고리즘 및 오프라인 큐 상세는 `23-OFFLINE_BEHAVIOR.md` (작성 예정) 위임.

**라운드 진행 중 저장 타이밍**: HoleScore는 탭 인터랙션 직후 SwiftData에 즉시 write한다. 별도 "저장" 버튼 없이 매 탭마다 영속화되므로, 앱 강제 종료 시에도 마지막 탭까지 보존된다 (spec_3.md:108-109).

### 3.2 Watch ↔ iPhone 점수 sync

spec §11 Step 7 (spec_3.md:866) 매핑: "iPhone ↔ Watch 점수 sync, 라운드 상태 sync, 충돌 처리"
...
**HealthKit 워크아웃 경로**: Watch 앱이 `HKWorkoutSession`을 시작/종료하며, 걸음 수/칼로리/심박수는 HealthKit 로컬 DB에만 기록된다 (spec_3.md:116-119). HealthKit 데이터는 SwiftData를 경유하지 않는다.

### 3.3 라운드 종료 → viewer 생성

spec §11 Step 12 (spec_3.md:880-887) 매핑: "Cloudflare Worker + KV + R2 (Viewer 백엔드)"

1. F-D 라운드 종료 → F-E 공유 옵션 모달 (실명/익명, 공개/PIN 선택)
2. POST `/api/share` 호출 — 라운드 데이터 + 사진 + 옵션 + 디바이스 토큰 (30-API_SPEC §3)
3. Worker가 shortId(base62 8자) 발급, KV(메타데이터) + R2(사진) write, 7일 TTL 설정 (spec_3.md:248-250)
...
**사진 업로드 경로**: 라운드 사진은 POST `/api/share` 본문에 직접 첨부하거나 POST `/api/share/{shortId}/photos`로 추가할 수 있다. 1장당 최대 10MB, viewer당 최대 30장 (spec_3.md:279-280). 업로드 실패 시 재시도 정책은 `23-OFFLINE_BEHAVIOR.md` 위임.
...
spec §11 Step 1 (spec_3.md:847-852)에 따라 **3타깃**으로 구성한다.

| 타깃 | 역할 | 출처 |
|------|------|------|
| **iOS 앱 (App-iOS)** | iPhone UI (SwiftUI Views), ViewModels, Repository 구현체, WatchConnectivity 클라이언트 | spec_3.md:848 |
| **watchOS 앱 (App-Watch)** | Apple Watch UI (SwiftUI Views), HealthKit 워크아웃 세션, Digital Crown 카운터 | spec_3.md:849 |
| **Shared** | SwiftData `@Model` 클래스, 디자인 토큰, DTO 타입, Repository 프로토콜 정의 | spec_3.md:850 |
...
| # | 근거 | 출처 |
|---|------|------|
| 1 | `courses.json` 실측 143KB(143,832 bytes) — 앱 시작 시 1회 디코딩 후 메모리 배열로 충분, 쿼리 성능 문제 없음 | 실측 (Ref-docs/golf-db-pack/courses.json) |
...
| 4 | Haversine 반경 3km 매칭(F1)은 546개 × O(1) 거리 계산으로 단일 호출당 1ms 미만 — 메모리 배열 선형 탐색 충분 | spec_3.md:58-61 |
...
`dataQuality` 기반 F3 홀 자동 감지 분기 처리 (524개 `low` / 14개 `high`)는 `21-DATA_MODEL.md` 위임.
...
- **정상 동작 근거**: spec_3.md:114 "로컬 우선, 네트워크 복구 시 자동 sync" 원칙과 일치. iCloud 미로그인 상태에서도 라운드 시작/저장/종료는 차단하지 않는다.
- **설정 화면 상단 배너**: "iCloud 미연결 — 다른 기기와 동기화되지 않습니다"
  - 배너 표시는 **spec 외 추가**: 53-PERMISSIONS §7 위임 결과이며, `spec_3.md` 원문에는 배너 명시 없음.
- **인앱 다이얼로그/강제 로그인 유도 금지**: 개인정보 톤 (CLAUDE.md §PROJECT — "위치/동반자 이름 외부 전송 금지")과 일치하는 비강제 정책. 사용자에게 계정 행동을 강요하지 않는다.
- **viewer 공유**: 디바이스 토큰 기반(spec_3.md:670)이므로 iCloud 로그인 여부와 무관하게 동작.
...
본 섹션은 `spec_3.md`에 없는 구현 권장안이며, 실제 결정은 구현 단계에서 확정. 본 문서가 명세화하지 않는다. 미채택 시 영향 없음.
...
- 테스트 커버리지 목표는 spec_3.md §11 Step 17 (spec_3.md:899-900) 참조.

---

## 10. [SPEC-UNDEFINED] 미정 항목

spec_3.md에 명시되지 않아 구현 단계에서 별도 결정이 필요한 항목이다.
