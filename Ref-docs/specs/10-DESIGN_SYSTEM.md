...
> **작성일**: 2026-05-11
> **버전**: v4 기반
> **출처 명세서**: [기능 명세서 v4](../golf-scorecard-app-spec_3.md) §5 (spec_3.md:385-483)

---

## 1. 디자인 원칙

라운드온의 시각 언어는 다음 4가지 지침을 동시에 따른다. (spec_3.md:387-394)

- **컨셉**: simple, 4계절의 그린(잔디) 느낌, 세련, 컴포넌트 절제 (spec_3.md:390)
- **Apple HIG**: clarity(명확성), deference(존중), depth(깊이) 세 원칙 준수 (spec_3.md:392)
- **Material Design 3**: Material You 기조의 표현적(expressive) 구성 참조 (spec_3.md:393)
- **Anthropic Frontend Design Skill**: intentional aesthetic, distinctive choices — 의도된 심미와 차별화된 선택 (spec_3.md:394)

> 목업: Google Stitch로 생성 예정 (spec_3.md:389). 본 문서 범위 외.

---

## 2. 컬러 팔레트 — 사계절 그린

앱 테마는 4계절 이름을 딴 4개 팔레트로 구성된다. 시스템 외관 설정에 따른 디폴트 매핑: 시스템 라이트 → Spring, 시스템 다크 → Winter. 설정 화면에서 4계절 수동 선택 가능. (spec_3.md:440)

### Spring — 봄 (라이트 디폴트)

시스템 라이트 모드의 기본 팔레트. (spec_3.md:398-408)
...
선명한 색감의 선택 팔레트. (spec_3.md:410-417)
...
따뜻한 황록 계열의 선택 팔레트. (spec_3.md:419-426)
...
시스템 다크 모드의 기본 팔레트. (spec_3.md:428-438)
...
라운드온의 핵심 기능인 F4 "0에서 시작" 카운터(CLAUDE.md §PROJECT)에서 점수 숫자를 크게 표시하기 위한 전용 토큰. (spec_3.md:460-462)
...
라운드온은 8pt 그리드를 기준 단위로 사용한다. (spec_3.md:474, CLAUDE.md §PROJECT)

### 간격 토큰

아래 xs–xl 레이블 매핑은 구현 제안이며, spec_3.md에는 수치만 명시되어 있다.
...
최소 원칙을 따르며 1–2단계만 사용한다. (spec_3.md:469) 그림자 offset / blur / opacity 정확한 수치는 `11-COMPONENTS.md`(작성 예정)에 위임한다.

### 테두리

- 두께: 1pt (spec_3.md:470)
- 색상: `--border` 토큰 (팔레트별 정의 §2 참조)

### 아이콘 / 버튼

- 아이콘: SF Symbols `.regular` 두께 사용 (spec_3.md:472)
- 버튼 변형: filled / tinted / plain 3종류만 허용 (spec_3.md:473)

---

## 6. 야외 가독성

골프장 야외 환경에서의 가독성 확보를 위한 필수 요건이다. (spec_3.md:478-483)

- 모든 화면에서 **최소 4.5:1 명도 대비** 확보 (WCAG AA 기준) (spec_3.md:480)
- 라운드 시작 시 "밝기 최대 권장" 안내를 한 번 표시한다 (spec_3.md:481)
- 점수 숫자는 항상 `weight: .semibold` 이상으로 표시한다 (spec_3.md:482)
...
본 섹션은 `spec_3.md`에 없는 구현 권장안이며, 실제 Asset 이름은 구현 시점 결정. 본 문서가 명세화하지 않는다.
...
다음 항목은 본 문서 작성 시점에 `spec_3.md`에 정의되지 않았거나 후속 문서에 위임된 내용이다.
