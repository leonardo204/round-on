...
| **HoleViewModel** | `currentHoleIndex: Int`, `currentPar: Int`, `gpsMatchState` | GPS 자동 홀 이동 (dataQuality `full` 14개 코스), 수동 스와이프 | spec_3.md:97 |
...
struct HoleChange: Codable {
    ...
    let trigger: ChangeTrigger  // .gpsAuto / .manualSwipe
    let timestamp: Date
    let deviceId: String
