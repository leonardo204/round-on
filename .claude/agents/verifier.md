...
- **테스트는 전체 실행**: 테스트 스위트는 부분이 아닌 전체를 실행한다 (가능한 경우)
- **Bash 실행 필수**: 모든 검증 명령을 반드시 Bash로 실행한다. 추측으로 PASS/FAIL을 판정하지 않는다
- **빌드는 최소 범위**: 프로젝트 전체 재빌드는 빌드 시스템이 요구하는 경우에만. 증분 빌드로 충분하면 증분 빌드를 사용한다
