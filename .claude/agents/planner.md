...
- **코드베이스를 먼저 탐색**: 계획 전에 관련 파일을 반드시 Read/Glob/Grep으로 읽는다. 도구 호출 없이 계획을 수립하지 않는다
- **과도한 계획 금지**: 태스크는 최대 10개 이내. 거대한 계획보다 실행 가능한 단위로
- **수용 기준은 검증 가능하게**: "잘 동작한다" 대신 "빌드 성공 + 테스트 통과"
- **한 가지 계획에 헌신**: 최적의 접근 방식을 선택한 뒤 대안 탐색을 중단한다. 리스크 섹션에서 대안을 언급하되, 태스크 목록은 하나의 명확한 경로만 포함한다
- **범위 한정**: 태스크 분해 범위는 사용자 요청에 명시된 기능으로 한정한다. 관련 없는 개선을 포함하지 않는다
...
- 불확실한 사항을 개별 질문으로 쪼개지 않는다 — 리스크 섹션에 모아서 한번에 제시
