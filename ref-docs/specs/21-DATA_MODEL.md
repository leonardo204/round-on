...
> **관련 문서**: [01-SPEC](01-SPEC.md) · [20-ARCHITECTURE](20-ARCHITECTURE.md) · [22-STATE_MANAGEMENT](22-STATE_MANAGEMENT.md) · [23-OFFLINE_BEHAVIOR](23-OFFLINE_BEHAVIOR.md) · [30-API_SPEC](30-API_SPEC.md) · [../golf-db-pack/40-COURSE_DB_SCHEMA.md](../golf-db-pack/40-COURSE_DB_SCHEMA.md) · [전체 인덱스](README.md)

> **작성일**: 2026-05-11
> **버전**: v4 기반
> **출처 명세서**: [기능 명세서 v4](01-SPEC.md) §6 (01-SPEC.md:486-560)
...
출처: 01-SPEC.md §6 (486-560), CLAUDE.md §PROJECT.

---

## 2. 모델 개요 다이어그램

아래 다이어그램은 01-SPEC.md:486-560에서 직접 도출한 모델 간 참조 관계다.
...
아래 4개 `@Model` 클래스는 01-SPEC.md:488-534에 정의된 Swift 코드를 기준으로 한다. `@Relationship` 어노테이션은 spec에 없으며, 연관 모델은 단순 배열로 보유한다 (spec 미정의 — 후속 보완 TODO).

### Round (01-SPEC.md:489-507)

| 필드 | 타입 | Optional | 출처 | 비고 |
|------|------|----------|------|------|
| id | UUID | 아니오 | 01-SPEC.md:491 | 라운드 식별자 |
| date | Date | 아니오 | 01-SPEC.md:492 | 라운드 날짜 |
| courseId | String | 아니오 | 01-SPEC.md:493 | GolfCourse.id 외래키 참조 |
| courseName | String | 아니오 | 01-SPEC.md:494 | 코스 표시명 (비정규화 복사) |
| courseSubName | String | 예 | 01-SPEC.md:495 | 수동 입력 또는 GPS 서브코스 감지 결과 (예: "동코스"). SubCourseSelector에서 선택한 SubCourse.name 저장. |
| players | [Player] | 아니오 | 01-SPEC.md:496 | 동반자 목록 |
| holes | [HoleScore] | 아니오 | 01-SPEC.md:497 | 홀별 점수 |
| photos | [RoundPhoto] | 아니오 | 01-SPEC.md:498 | 첨부 사진 |
| isFinished | Bool | 아니오 | 01-SPEC.md:499 | 라운드 종료 여부 |
| startedAt | Date | 아니오 | 01-SPEC.md:500 | 라운드 시작 시각 |
| finishedAt | Date | 예 | 01-SPEC.md:501 | 라운드 종료 시각 |
| sharedShortId | String | 예 | 01-SPEC.md:502 | Viewer URL 단축 ID |
| sharedURL | String | 예 | 01-SPEC.md:503 | 전체 Viewer URL |
| sharedExpiresAt | Date | 예 | 01-SPEC.md:504 | Viewer 만료 시각 (7일 후) |
| sharedEditToken | String | 예 | 01-SPEC.md:505 | Viewer 수정 토큰 |
| sharedOptions | ShareOptions | 예 | 01-SPEC.md:506 | 이름 공개 여부 + 접근 제어 |

### Player (01-SPEC.md:510-515)

| 필드 | 타입 | Optional | 출처 | 비고 |
|------|------|----------|------|------|
| id | UUID | 아니오 | 01-SPEC.md:511 | HoleScore의 counts 키 |
| name | String | 아니오 | 01-SPEC.md:512 | 별명(닉네임) — 실명 외부 전송 금지 |
| isOwner | Bool | 아니오 | 01-SPEC.md:513 | 라운드 소유자(기기 사용자) 여부 |
| order | Int | 아니오 | 01-SPEC.md:514 | 표시 순서 |

### HoleScore (01-SPEC.md:518-524)

| 필드 | 타입 | Optional | 출처 | 비고 |
|------|------|----------|------|------|
| holeNumber | Int | 아니오 | 01-SPEC.md:519 | 홀 번호 (1-18) |
| par | Int | 아니오 | 01-SPEC.md:520 | 해당 홀 파 |
| counts | [UUID: Int] | 아니오 | 01-SPEC.md:521 | Player.id → 총 타수 |
| obCount | [UUID: Int] | 아니오 | 01-SPEC.md:522 | Player.id → OB 벌타 수 |
| hazardCount | [UUID: Int] | 아니오 | 01-SPEC.md:523 | Player.id → 해저드 벌타 수 |

`counts`, `obCount`, `hazardCount` 세 필드 모두 `[UUID: Int]` 타입이며 Player.id를 키로 사용한다 (01-SPEC.md:521-523). `Dictionary<UUID, Int>`의 SwiftData 직렬화 지원 여부는 §10 — `[UUID: Int]` SwiftData 영속화 검증 필요 절 참조.

### RoundPhoto (01-SPEC.md:527-534)

| 필드 | 타입 | Optional | 출처 | 비고 |
|------|------|----------|------|------|
| id | UUID | 아니오 | 01-SPEC.md:528 | 사진 식별자 |
| localPath | String | 아니오 | 01-SPEC.md:529 | 기기 내 로컬 경로 |
| remoteURL | String | 예 | 01-SPEC.md:530 | Cloudflare R2 업로드 URL |
| capturedAt | Date | 아니오 | 01-SPEC.md:531 | 촬영 시각 |
| holeNumber | Int | 예 | 01-SPEC.md:532 | 연관 홀 번호 |
| caption | String | 예 | 01-SPEC.md:533 | 사진 설명 |

### 값 타입: ShareOptions / HoleInfo

`ShareOptions`와 `HoleInfo`는 `@Model`이 아닌 Codable struct다 (01-SPEC.md:536-559).
...
01-SPEC.md §6에 `@Relationship` 어노테이션 정의 없음 (spec 미정의).
...
### GolfCourse @Model (01-SPEC.md:542-550)
...
| 필드 | 타입 | Optional | 출처 |
|------|------|----------|------|
| id | String | 아니오 | 01-SPEC.md §6 |
| name | String | 아니오 | 01-SPEC.md §6 |
| region | String | 아니오 | 01-SPEC.md §6 |
| clubhouseLat | Double | 아니오 | 01-SPEC.md §6 |
| clubhouseLng | Double | 아니오 | 01-SPEC.md §6 |
...
| holes | [HoleInfo] | 아니오 | 01-SPEC.md §6 |
...
01-SPEC.md §6은 `GolfCourse`를 `@Model`로 정의하고, golf-db-pack은 번들 JSON 방식으로 제공한다. 두 정의가 병존한다. 어느 방식을 채택할지(또는 둘 다 사용할지)는 `20-ARCHITECTURE.md`에서 결정한다 — 결정 시 고려 기준: 메모리/번들 사이즈, CloudKit private DB와의 sync 범위 충돌 여부, 코스 데이터 갱신 빈도. 본 문서는 단정적 권장을 하지 않는다.

Round 모델은 `courseId: String` 외래키로 코스를 참조하며, 어느 방식을 채택하더라도 동일하게 동작한다.

### JSON ↔ Swift 필드 매핑 요약

번들 JSON(`courses.json`)의 키와 `GolfCourse` Swift 모델 필드 매핑. 상세 JSON 스키마는 [40-COURSE_DB_SCHEMA.md](../golf-db-pack/40-COURSE_DB_SCHEMA.md) 참조.

| JSON 키 | Swift 필드 | 타입 | 비고 |
|---------|-----------|------|------|
| `id` | `id` | String | 골프장 식별자 (이름 기반 slug) |
| `name` | `name` | String | 골프장 표시명 |
| `region` | `region` | String | 광역시도 (경기, 강원 등) |
| `clubhouse.lat` | `clubhouseLat` | Double | 클럽하우스 위도 |
| `clubhouse.lng` | `clubhouseLng` | Double | 클럽하우스 경도 |
| `holesCount` | `holesCount` | Int? | nil: 638곳 미기재 → 라운드 시작 시 사용자 선택 |
| `courseType` | `courseType` | String? | "CC", "GC" 등 |
| `phone` | `phone` | String? | 카카오 enrichment |
| `kakaoPlaceUrl` | `kakaoPlaceUrl` | String? | 카카오 장소 URL |
| `subCourses` | `subCourses` | [SubCourse]? | 서브코스 목록 (v3: 라벨만, 좌표 미포함) |
| `holes` | `holes` | [HoleInfo] | complete/partial/minimal만 비어있지 않음 |
| `dataQuality` | `dataQuality` | DataQuality | "complete"/"partial"/"minimal"/"low"/"unknown" |

---

## 6. CloudKit 매핑

01-SPEC.md:111-114 (F7) 에 따르면 라운드 기록은 CloudKit으로 자동 동기화된다.

- iOS 17 SwiftData + CloudKit 자동 sync 지원
- private database 사용 (라운드 기록은 개인 정보)
- 정책: 로컬 우선(local-first), 네트워크 복구 시 자동 sync (01-SPEC.md:114)
- 여러 디바이스에서 동일 데이터 확인 가능 (01-SPEC.md:113)
...
01-SPEC.md:116-119 (F8) 에 따르면 라운드를 `HKWorkoutActivityType.golf` 워크아웃으로 기록한다. 걸음 수, 칼로리, 심박수, 라운드 시간은 Apple Watch에서 자동 수집된다.

**Round 모델에 HealthKit UUID 매핑 필드 없음**: 01-SPEC.md §6 정의 기준, Round에 HKWorkout UUID를 저장하는 필드가 없다 (spec 미정의). HKWorkout과의 영속 매핑이 필요하다면 Round 모델에 필드 추가가 필요하다 — 후속 보완 TODO.
...
Round 모델의 공유 관련 4개 필드 (01-SPEC.md:502-505):
...
본 섹션은 `01-SPEC.md`에 없는 구현 권장안이며, 실제 결정은 `20-ARCHITECTURE.md` / `23-OFFLINE_BEHAVIOR.md` 등 후속 문서에서 확정한다. 본 문서가 명세화하지 않는다.
...
HoleScore의 `counts`, `obCount`, `hazardCount` 세 필드는 모두 `[UUID: Int]` (Dictionary) 타입이다 (01-SPEC.md:521-523). iOS 17 `@Model`에서 `Dictionary<UUID, Int>` 직렬화 지원 여부는 구현 단계에서 반드시 검증해야 한다.
